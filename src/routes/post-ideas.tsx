...
function PostIdeasPage() {
  ...
  /** The user can also write their own idea from scratch and send it the same way. */
  const [writingOwnIdea, setWritingOwnIdea] = useState(false);
  ...
  function confirmOwnIdea(hook: string, note: string) {
    addIdeaRequest({ hook, pillar: "Your idea" }, note);
    setWritingOwnIdea(false);
    toast.success("Idea sent to the Isla team");
    navigate({ to: "/approvals" });
  }
            ...
            {stage === "manager" && (
              ...
              >
                <IdeasFeedStage
                  ...
                  onWriteOwn={() => setWritingOwnIdea(true)}
                />
                <SendToTeamDialog
                  idea={sendingIdea}
                ...
                <WriteOwnIdeaDialog
                  open={writingOwnIdea}
                  onClose={() => setWritingOwnIdea(false)}
                  onConfirm={confirmOwnIdea}
                />
  ...
  return (
    <div>
      ...
      <div className="mb-8 flex flex-wrap items-center justify-between gap-4 border-b border-border pb-2">
        ...
        <Button variant="outline" onClick={onWriteOwn} className="shrink-0">
...
function WriteOwnIdeaDialog({
  ...
  open: boolean;
  onClose: () => void;
  onConfirm: (hook: string, note: string) => void;
}) {
  ...
  return (
    <Dialog open={open} onOpenChange={(v) => !v && onClose()}>
      <DialogContent className="max-w-md">
        <DialogHeader>
          <DialogTitle>Write your own idea</DialogTitle>
          <DialogDescription>
            Send it to the Isla team and they'll turn it into a post for your approval.
          </DialogDescription>
        </DialogHeader>
        <div className="space-y-1.5">
          <label htmlFor="own-idea-hook" className="text-xs font-medium">
            Your idea
          </label>
          <Textarea
            id="own-idea-hook"
            ...
            rows={4}
            maxLength={500}
            placeholder="e.g. Why we stopped chasing MQLs and what we track instead."
            className="resize-none"
            autoFocus
          />
        </div>
        <div className="space-y-1.5">
          <label htmlFor="own-idea-note" className="text-xs font-medium">
            Add a note for the team <span className="text-muted-foreground">(optional)</span>
          </label>
          <Textarea
            id="own-idea-note"
        ...
        <DialogFooter className="gap-2 sm:gap-0">
          ...
          <Button
            ...
            disabled={!hook.trim()}
            onClick={() => onConfirm(hook.trim(), note)}
          >
            Send to Isla team
          </Button>
        </DialogFooter>
